...
export class BanCommand extends ImperiaCommand {
    ...
    public async chatInputRun(interaction: ImperiaCommand.ChatInputCommandInteraction): Promise<InteractionResponse> {
        ...
        if (user.id === interaction.user.id) {
            throw new UserError({
                identifier: ImperiaIdentifiers.CommandServiceError,
                message: "( ノ・・)ノ You cannot ban yourself.",
            });
        }
        ...
        if (user.id === interaction.guild?.ownerId) {
            throw new UserError({
                identifier: ImperiaIdentifiers.CommandServiceError,
                message: "( ノ・・)ノ You cannot ban the server owner.",
            });
        }
        ...
        if (
            ...
            throw new UserError({
                identifier: ImperiaIdentifiers.CommandServiceError,
                message: "( ノ・・)ノ You cannot ban this user.",
            });
        }
        ...
        if (!member.bannable) {
            throw new UserError({
                identifier: ImperiaIdentifiers.CommandServiceError,
                message: "( ノ・・)ノ I cannot ban this user.",
            });
        }
        ...
        return interaction.reply({
            content: `˖ ݁𖥔 ݁˖ Successfully banned **${user.tag}** from the server.`,
        });
    }
    ...
    public async messageRun(message: Message, args: Args): Promise<Message> {
        ...
        if (reasonArgument.isErr()) {
            throw new UserError({
                identifier: ImperiaIdentifiers.ArgsMissing,
                message: "(゜-゜) You didn't provide any reason for banning the user.",
            });
        }
        ...
        member.send({
            embeds: [
                new ImperiaEmbedBuilder()
                    .setColorScheme("warning")
                    .setDescription(
                        `You have been banned from **${message.guild.name}** for the following reason: ${reason}`,
                    ),
            ],
        });
